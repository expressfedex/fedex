...
// const path = require('path'); // Not needed for local file paths anymore
const nodemailer = require('nodemailer');
const serverless = require('serverless-http'); // For converting Express app to serverless function
...
// These credentials must be set as Environment Variables in your Netlify dashboard
cloudinary.config({
...
const storage = new CloudinaryStorage({
    ...
    params: {
        folder: 'fedex-uploads', // Specify a folder in your Cloudinary account
        format: async (req, file) => 'pdf', // Force PDF format, adjust as needed ('png', 'jpeg' etc.)
...
// --- MongoDB Connection ---
// Cached connection for efficiency in serverless environment
let cachedDb = null;
...
// --- Middleware ---
const corsOptions = {
    origin: process.env.FRONTEND_URL, // Use the FRONTEND_URL from Netlify Environment Variables
...
// --- IMPORTANT: Removed local static file serving for '/uploads' ---
...
// (Ideally, these would be in separate files in a 'models' directory and required here)
...
const TrackingSchema = new mongoose.Schema({
    ...
    attachedFileName: { type: String, default: null }, // Stores the Cloudinary public_id for lookup
...
// --- Initial Data Population (Optional, for testing/initial setup) ---
// This function will run on every cold start of a function.
// For production, consider using a separate script to populate data once.
async function populateInitialData() {
    try {
        ...
        const newTracking = new Tracking({
            ...
            expectedDelivery: new Date('2025-07-12T00:00:00Z'), // Example date
...
app.use(async (req, res, next) => {
    try {
        ...
        await populateInitialData(); // Consider removing or making conditional in production
        next();
    } catch (err) {
...
app.post('/api/admin/upload-package-file', authenticateAdmin, upload.single('packageFile'), async (req, res) => {
    ...
    if (!trackingId) {
        // If trackingId is missing, delete the uploaded file from Cloudinary
    ...
    try {
        ...
        if (tracking.attachedFileName) {
            try {
                await cloudinary.uploader.destroy(tracking.attachedFileName); // attachedFileName stores public_id
            } catch (destroyError) {
                console.warn(`Could not delete old Cloudinary file ${tracking.attachedFileName}:`, destroyError.message);
                // Continue even if old file deletion fails, don't block new upload
            }
        }
        ...
        // Clean up newly uploaded file from Cloudinary if linking fails
        if (req.file && req.file.filename) {
            await cloudinary.uploader.destroy(req.file.filename);
        }
...
app.post('/api/admin/send-email', authenticateAdmin, upload.single('attachment'), async (req, res) => {
    ...
    try {
        ...
        // Clean up new file if email sending failed
        if (req.file) {
            await cloudinary.uploader.destroy(req.file.filename);
        }
    ...
    try {
        ...
        if (trackingToDelete.attachedFileName) {
            try {
                ...
                console.warn(`Could not delete Cloudinary file ${trackingToDelete.attachedFileName}:`, destroyError.message);
                // Continue even if deletion fails, don't block tracking record deletion
            }
        }
...
// --- IMPORTANT: Remove static HTML routes ---
// Your frontend (in the `public` folder) will be served by Netlify's static hosting.
// These Express routes are no longer needed for serving HTML files.
/*
...
// --- Universal 404 handler for API routes ---
// This middleware should only apply if no other API routes matched.
// For unmatched paths *outside* of /api, Netlify's redirects in netlify.toml will handle it.
...
app.use((err, req, res, next) => {
    ...
    res.status(err.statusCode || 500).json({
        ...
        // Only send stack trace in development, not production, for security
...
// This is the crucial part that makes your Express app compatible with Netlify Functions.
